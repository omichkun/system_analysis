...
**Менеджмент заказов, Контроль качества работ, Склад и инвентаризация, Печенья с предсказаниями, Система скидок** – (**Main App**) – один большой сервис, потому что кроме внезапно большой нагрузки на менеджмент заказов, других ограничений нет. 
...
Main App – модульный монолит, так как внутри несколько ограниченных контекстов. Было б неплохо, если б они не смешивались между собой окончательно.
...
## 001 Выбор архитектурного стиля для Make Cats Free

## Статус
Принят

## Контекст
Нам были предоставлен список функциональных и нефункциональных требований. На основе этого списка были составлены визуализации бизнес-процессов (посредством проведения [сессии ES](./3hw.drawio)), были определены [субдомены](#subdomains) бизнеса, а также ограниченные контексты. 

Было определено 10 [ограниченных контекстов](#bounded-contexts-with-subdomains), для которых были определены взаимосвязи, стриминги данных и бизнес-события. А также определены [консерны](#консерны-стейкхолдеров) стейкхолдеров. 


## Решение 
На основании полученных данных и сделанных выводов было определено, что в приложении Make Cats Free (MCF) будет использоваться микросервисная архитектура с преимущественно асинхронными связями. Во всех сервисах используем отдельные RDBMS, кроме сервиса подбора воркера. Там используем графовую БД, так как там предполагается хранение неструктурированных данных, а также нахождение связей между ними.
