...
| Менеджмент заказов | Generic | Создание и управление заказом | simplicity, scalability, elasticity | **simplicity** – клиентам должно быть понятно и просто пользоваться системой, иначе уйдут к конкурентам. **scalability, elasticity** – из-за внезапной популярности приложения. Нужно держать нагрузку как среднюю, так и пиковую. |
